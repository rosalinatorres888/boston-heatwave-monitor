...
if st.sidebar.checkbox("Auto Refresh (30s)", value=False):
    ...
    st.session_state.update_count += 1
    st.rerun()"""
Boston Heatwave Live Monitoring Dashboard
A professional web-based real-time monitoring system
...
# Initialize session state
if 'api_key' not in st.session_state:
    st.session_state.api_key = "bcd5dd89868c05fe0807fa4eb99370b0"  # Your API key
if 'update_count' not in st.session_state:
    st.session_state.update_count = 0
if 'historical_data' not in st.session_state:
    st.session_state.historical_data = []
...
with st.sidebar:
    ...
    show_map = st.checkbox("Show Heat Map", value=True)
    ...
    heat_threshold = st.slider("Heat Index Alert (°F)", 80, 110, 95)
    uv_threshold = st.slider("UV Index Alert", 5, 11, 8)
...
# Main Header
st.markdown("<h1>🌡️ Boston Heatwave Live Monitor</h1>", unsafe_allow_html=True)
...
with col2:
    st.markdown(
        ...
        f'<span class="live-indicator">🔴 LIVE</span> '
        f'<span style="margin-left: 20px;">Last Update: {datetime.now().strftime("%I:%M:%S %p")}</span>'
        f'</div>',
        unsafe_allow_html=True
    )
...
# Fetch current weather data
current_weather = weather.get_current_weather()
forecast_data = weather.get_forecast() if show_forecast else None
air_quality = weather.get_air_quality() if show_alerts else None
...
if current_weather:
    ...
    with metric_cols[5]:
        st.metric(
            "👁️ Visibility",
            f"{current_weather.get('visibility', 10000)/1609.34:.1f} mi",
    ...
    if show_alerts:
        ...
        with alert_cols[0]:
            if heat_index >= heat_threshold:
                alert_class = "heat-extreme" if heat_index >= 105 else "heat-high"
                st.markdown(
                    f'<div class="alert-box {alert_class}">'
                    f'<strong>⚠️ HEAT ADVISORY</strong><br>'
                    f'Heat Index: {heat_index}°F<br>'
                    f'Stay hydrated, seek cooling'
                    f'</div>',
                    unsafe_allow_html=True
                )
            else:
                st.markdown(
                    '<div class="alert-box heat-safe">'
                    '<strong>✅ Heat Levels Safe</strong><br>'
                    'No heat advisory active'
                    '</div>',
                    unsafe_allow_html=True
                )
        ...
        with alert_cols[1]:
            if air_quality:
                ...
                aqi_colors = {1: "heat-safe", 2: "heat-moderate", 3: "heat-moderate", 4: "heat-high", 5: "heat-extreme"}
                
                st.markdown(
                    f'<div class="alert-box {aqi_colors[aqi]}">'
                    f'<strong>🌫️ Air Quality: {aqi_labels[aqi]}</strong><br>'
                    f'AQI Level: {aqi}<br>'
                    f'PM2.5: {air_quality["list"][0]["components"]["pm2_5"]:.1f} μg/m³'
                    f'</div>',
                    unsafe_allow_html=True
                )
        ...
        with alert_cols[2]:
            ...
            st.markdown(
                f'<div class="alert-box heat-moderate">'
                f'<strong>☁️ Weather Status</strong><br>'
                f'{desc}<br>'
    ...
    with chart_cols[0]:
        ...
        if len(st.session_state.historical_data) > 1:
            ...
            # Add danger zones
            fig_temp.add_hrect(y0=105, y1=150, fillcolor="red", opacity=0.1, annotation_text="Extreme")
            fig_temp.add_hrect(y0=90, y1=105, fillcolor="orange", opacity=0.1, annotation_text="High")
            ...
            fig_temp.update_layout(
                ...
                yaxis=dict(title="Temperature (°F)", showgrid=True, gridcolor='rgba(255,255,255,0.1)')
            )
            
            st.plotly_chart(fig_temp, use_container_width=True)
        else:
            st.info("Collecting data... Chart will appear after multiple readings")
    ...
    with chart_cols[1]:
        ...
        if len(st.session_state.historical_data) > 1:
            ...
            fig_env.update_xaxes(showgrid=False)
            fig_env.update_yaxes(title_text="Humidity (%)", secondary_y=False, showgrid=True, gridcolor='rgba(255,255,255,0.1)')
            fig_env.update_yaxes(title_text="Pressure (hPa)", secondary_y=True)
    ...
    if show_forecast and forecast_data:
        ...
        for item in forecast_data['list'][:8]:  # Next 24 hours
            forecast_times.append(datetime.fromtimestamp(item['dt']))
            forecast_temps.append(item['main']['temp'])
            forecast_feels.append(item['main']['feels_like'])
            forecast_humidity.append(item['main']['humidity'])
            forecast_rain.append(item.get('pop', 0) * 100)
        ...
        # Rain probability (secondary axis)
        fig_forecast.add_trace(go.Bar(
            ...
            width=1000*60*60*2  # 2 hours in milliseconds
        ))
        ...
        with summary_cols[0]:
            max_temp = max(forecast_temps)
            st.info(f"**🌡️ Max Temperature**\n\n{max_temp:.1f}°F")
        
        with summary_cols[1]:
            min_temp = min(forecast_temps)
            st.info(f"**❄️ Min Temperature**\n\n{min_temp:.1f}°F")
        
        with summary_cols[2]:
            avg_humidity = np.mean(forecast_humidity)
            st.info(f"**💧 Avg Humidity**\n\n{avg_humidity:.0f}%")
        
        with summary_cols[3]:
            max_rain = max(forecast_rain)
            st.info(f"**🌧️ Max Rain Chance**\n\n{max_rain:.0f}%")
    
    # Heat Map Section
    if show_map:
        st.markdown("## 🗺️ Boston Neighborhood Heat Islands")
        
        # Create neighborhood data
        neighborhoods = {
            'Downtown': {'lat': 42.3554, 'lon': -71.0640, 'heat_factor': 1.4},
            'Chinatown': {'lat': 42.3516, 'lon': -71.0629, 'heat_factor': 1.5},
            'Roxbury': {'lat': 42.3317, 'lon': -71.0863, 'heat_factor': 1.35},
            'Dorchester': {'lat': 42.3010, 'lon': -71.0674, 'heat_factor': 1.3},
            'East Boston': {'lat': 42.3784, 'lon': -71.0390, 'heat_factor': 1.35},
            'Back Bay': {'lat': 42.3503, 'lon': -71.0810, 'heat_factor': 1.2},
            'Jamaica Plain': {'lat': 42.3097, 'lon': -71.1153, 'heat_factor': 1.1},
            'Brighton': {'lat': 42.3506, 'lon': -71.1589, 'heat_factor': 1.05},
            'Charlestown': {'lat': 42.3782, 'lon': -71.0602, 'heat_factor': 1.25},
            'South End': {'lat': 42.3389, 'lon': -71.0768, 'heat_factor': 1.3}
        }
        
        # Calculate temperatures for each neighborhood
        neighborhood_data = []
        for name, info in neighborhoods.items():
            est_temp = temp * info['heat_factor']
            est_heat_index = weather.calculate_heat_index(est_temp, humidity)
            
            neighborhood_data.append({
                'Neighborhood': name,
                'Temperature': est_temp,
                'Heat Index': est_heat_index,
                'Heat Factor': info['heat_factor'],
                'lat': info['lat'],
                'lon': info['lon']
            })
        ...
        fig_map = px.scatter_mapbox(
            ...
            color='Temperature',
            text='Neighborhood',
            hover_data=['Temperature', 'Heat Index', 'Heat Factor'],
        ...
        fig_map.update_layout(
            ...
            mapbox=dict(center=dict(lat=42.3601, lon=-71.0589)),
        ...
        with col1:
            st.markdown("### 🔥 Hottest Neighborhoods")
            df_hot = df_neighborhoods.nlargest(5, 'Temperature')[['Neighborhood', 'Temperature', 'Heat Index']]
            df_hot['Temperature'] = df_hot['Temperature'].round(1).astype(str) + '°F'
            df_hot['Heat Index'] = df_hot['Heat Index'].round(0).astype(int).astype(str) + '°F'
            st.dataframe(df_hot, hide_index=True, use_container_width=True)
        
        with col2:
            st.markdown("### ❄️ Coolest Neighborhoods")
            df_cool = df_neighborhoods.nsmallest(5, 'Temperature')[['Neighborhood', 'Temperature', 'Heat Index']]
            df_cool['Temperature'] = df_cool['Temperature'].round(1).astype(str) + '°F'
            df_cool['Heat Index'] = df_cool['Heat Index'].round(0).astype(int).astype(str) + '°F'
            st.dataframe(df_cool, hide_index=True, use_container_width=True)
    
    # Historical Analysis
    if show_history and len(st.session_state.historical_data) > 10:
        ...
        with analysis_cols[1]:
            max_heat = df_history['heat_index'].max()
            st.metric("Max Heat Index", f"{max_heat:.0f}°F")
        
        with analysis_cols[2]:
            temp_trend = df_history['temp'].iloc[-1] - df_history['temp'].iloc[0]
            st.metric("Temperature Trend", f"{temp_trend:+.1f}°F")
    ...
    st.markdown(
        ...
        f'Data Source: OpenWeatherMap API | '
        f'Location: Boston, MA ({weather.boston_lat}, {weather.boston_lon}) | '
        f'Updates: Every {refresh_rate} seconds'
        f'</div>',
        unsafe_allow_html=True
    )

else:
    st.error("⚠️ Unable to fetch weather data. Please check your API key and connection.")
